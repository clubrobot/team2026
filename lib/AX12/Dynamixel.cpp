...
#define readData()      (AX12_SERIAL.read())         // Read Serial Data
#define peekData()      ((uint8_t) AX12_SERIAL.peek())         // Peek Serial Data
#define beginCom(args)  (AX12_SERIAL.begin(args))    // Begin Serial Comunication
#define endCom()        (AX12_SERIAL.end())          // End Serial Comunication
#define setDuplexRX()   (AX12_SERIAL.enableHalfDuplexRx()) //Enables RX of the halfDuplex. Disabled by write
#define enableHalfDuplex() (AX12_SERIAL.setHalfDuplex()) //Set HalfDuplex before begin

#define setTXPin(pin)			(AX12_SERIAL.setTx(pin))
...
void DynamixelClass::begin(long baud, unsigned char RxTx) {
	DTx = RxTx;
	setTXPin(RxTx);
	enableHalfDuplex();
	beginCom(baud);
...
int DynamixelClass::moveSpeed(unsigned char ID, int Position, int Speed) {
	...
	Checksum = (~(ID + AX_GOAL_SP_LENGTH + AX_WRITE_DATA + AX_GOAL_POSITION_L + Position_L + Position_H + Speed_L +
		Speed_H)) & 0xFF;
...
int DynamixelClass::moveSpeedRW(unsigned char ID, int Position, int Speed) {
	...
	Speed_H = Speed >> 8;
	Speed_L = Speed; // 16 bits - 2 x 8 bits variables
	Checksum = (~(ID + AX_GOAL_SP_LENGTH + AX_REG_WRITE + AX_GOAL_POSITION_L + Position_L + Position_H + Speed_L +
		Speed_H)) & 0xFF;
...
int DynamixelClass::setAngleLimit(unsigned char ID, int CWLimit, int CCWLimit) {
	...
	CCW_H = CCWLimit >> 8;
	CCW_L = CCWLimit;
	Checksum = (~(ID + AX_VL_LENGTH + AX_WRITE_DATA + AX_CW_ANGLE_LIMIT_L + CW_H + CW_L + AX_CCW_ANGLE_LIMIT_L + CCW_H +
		CCW_L)) & 0xFF;
...
int DynamixelClass::setCMargin(unsigned char ID, unsigned char CWCMargin, unsigned char CCWCMargin) {
	Checksum = (~(ID + AX_CM_LENGTH + AX_WRITE_DATA + AX_CW_COMPLIANCE_MARGIN + CWCMargin + AX_CCW_COMPLIANCE_MARGIN +
		CCWCMargin)) & 0xFF;
...
int DynamixelClass::setCSlope(unsigned char ID, unsigned char CWCSlope, unsigned char CCWCSlope) {
	Checksum = (~(ID + AX_CS_LENGTH + AX_WRITE_DATA + AX_CW_COMPLIANCE_SLOPE + CWCSlope + AX_CCW_COMPLIANCE_SLOPE +
		CCWCSlope)) & 0xFF;
