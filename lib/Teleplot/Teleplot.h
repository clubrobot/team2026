...
public:
    ...
    public :
        ...
        explicit ShapeValue(double val) : isSet(true), value(val) {};

        bool isSet;
        double value;
        [[nodiscard]] std::string valueRounded() const
        {
            return roundValue(value);
        }

    private :
        [[nodiscard]] static std::string roundValue(const double value)
        {
    ...
    ShapeTeleplot(std::string  name, std::string  type, std::string  color = "") : _name(std::move(name)), _type(std::move(type)), _color(std::move(color)){};
    ...
    [[nodiscard]] std::string toString() const
    {
        ...
        if (!_color.empty()) { result << ":C:" << _color; }
...
class Teleplot {
public:
    explicit Teleplot(unsigned int bufferingFrequencyHz = 30)
    ...
    void update3D(ShapeTeleplot const& mshape, unsigned int maxFrequencyHz = 0, const std::string& flags = TELEPLOT_FLAG_DEFAULT) {
#ifdef TELEPLOT_DISABLE
        return ;
#endif
        int64_t nowUs = std::chrono::time_point_cast<std::chrono::microseconds>(std::chrono::system_clock::now()).time_since_epoch().count();
...
private:
    ...
    static std::string formatPacket(std::string const &key, std::string const& values, std::string const& flags, const std::string& unit, bool is3D = false){
        std::ostringstream oss;
        std::string unitFormatted = (unit.empty()) ? "" : "§" + unit;
    ...
    static void emit(std::string const& data){
        printf("%s", data.c_str());
    }

#ifdef TELEPLOT_USE_BUFFERING
    void buffer(std::string const &key, std::string const& values, std::string const& flags, const std::string& unit, bool is3D = false) {
